...
public class TerraformClient implements AutoCloseable {
    ...
    private ProcessLauncher getTerraformLauncher(String command) throws IOException {
        ...
        launcher.setEnvironmentVariable(SUBS_ID_ENV_NAME, this.options.getSubscriptionId());
        launcher.setEnvironmentVariable(CLIENT_ID_ENV_NAME, this.options.getClientId());
        launcher.setEnvironmentVariable(SECRET_ENV_NAME, this.options.getClientSecret());
        launcher.setEnvironmentVariable(TENANT_ID_ENV_NAME, this.options.getTenantId());
